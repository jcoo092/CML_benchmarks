...
fun communicate iterations count sendEvts i recvCh = let
    val recv_evt = PrimChan.recvEvt recvCh
    fun runwhisper 0 = ()
...
fun ring iterations num_threads = let
    ...
    fun interpose in_ch out_ch = 
        (case PrimChan.recv in_ch of
            0  => ()
          | msg => (
              PrimChan.send(out_ch, (msg - 1));
              interpose in_ch out_ch))

    val chans = List.tabulate(num_threads, fn _ => PrimChan.new ())
    val thds = ListPair.map (
			fn (a, b) => spawn (fn () => recv_and_fwd a b))
		(chans, (List.tl chans))
    val interpose_thd = spawn (
		fn () => interpose (List.last chans) (List.hd chans))
in
    PrimChan.send((List.hd chans), iterations);
...
fun kn iterations num_threads = let
    ...
    val part_send_evts = List.map (fn c => fn v =>
                                      CML.wrap (PrimChan.sendEvt (c, v), fn _ => v)) chans
    val thds = ListPair.mapEq
                   (fn (i, c) => let
                    ...
                    in
                        spawn (fn () => communicate iterations count sendEvts i c)
...
fun grid iterations width height = let
    ...
    val thds = ListPair.mapEq (fn (sends, (i, c)) => let
                                   val sendEvts = List.map (fn s => s i) sends
                               in spawn (fn () => communicate iterations
...
fun experiment experiment_selection iterations num_threads width height () = 
