...
(define (vector iterations size)
  ...
  (: initvector1 (Array Nonnegative-Fixnum))
  (define initvector1 (build-matrix 1 size (λ (a b) (rando))))
  (: initvector2 (Array Nonnegative-Fixnum))
  (define initvector2 (build-matrix 1 size (λ (a b) (rando))))

  (: process-vectors (-> Nonnegative-Fixnum (Array Nonnegative-Fixnum) (Array Nonnegative-Fixnum) Void))
  (define (process-vectors iteration v1 v2)
    (if (zero? iteration)
        (void)
        (let ([pluses (cast (matrix+ v1 v2) (Array Nonnegative-Fixnum))]
              [times (cast (matrix-row (matrix* (matrix-transpose v1) v2) 0) (Array Nonnegative-Fixnum))])
...
(define (matrixops iterations size)
  ...
  (: process-matrices (-> Nonnegative-Fixnum (Array Nonnegative-Fixnum) (Array Nonnegative-Fixnum) Void))
  (define (process-matrices iteration m1 m2)
    (if (zero? iteration)
        (void)
        (let ([pluses (cast (matrix+ m1 m2) (Array Nonnegative-Fixnum))]
              [times (cast (matrix* m1 m2) (Array Nonnegative-Fixnum))])
...
(define (mixed iterations size)
  ...
  (: process-mixed (-> Nonnegative-Fixnum (Array Nonnegative-Fixnum) (Array Nonnegative-Fixnum) (Array Nonnegative-Fixnum) Void))
  (define (process-mixed iteration colvec rowvec m)
    (if (zero? iteration)
        (void)
        (let* ([next-iter (sub1 iteration)]
               [next-colvec (rem-all-by-256 (cast (matrix* m colvec) (Array Nonnegative-Fixnum)))]
               [next-rowvec (rem-all-by-256 (cast (matrix* rowvec m) (Array Nonnegative-Fixnum)))]
               [next-matrix (rem-all-by-256 (cast (matrix* next-colvec next-rowvec) (Array Nonnegative-Fixnum)))])
          (process-mixed next-iter next-colvec next-rowvec next-matrix))))
  (process-mixed iterations initcolvec initrowvec initmatrix1))

;***************************

(: main (-> String String String Void))
(define (main experiment-selection iterations size)
  (let ([size-num (cast (string->number size) Nonnegative-Fixnum)]
        [iter-num (cast (string->number iterations) Nonnegative-Fixnum)])
    (cast iter-num Nonnegative-Fixnum)
